...
PROJECT_NAME = "DP-FedAug-MNIST-Study-seeded"
OUTPUT_DIR = Path(__file__).resolve().parents[1] / "outputs" / "mnist_dpfedaug_seeded"
...
def plot_accuracy_vs_synth_by_alpha(summary_df: pd.DataFrame, output_dir: Path):
    """Line plot: x=synthetic_count, y=accuracy, one line per alpha.
    Shows that synthetic data helps most for non-IID and can hurt for IID.
    One figure per total_n. Faceted by epsilon within each figure.
    """
    apply_thesis_style()
    ...
    for total_n, n_df in df.groupby("total_n"):
        ...
        for ax, eps in zip(axes, epsilons):
            ...
            eps_title = f"No DP" if eps == "none" else f"$\\varepsilon$={eps}"
        ...
        fig.suptitle(f"Effect of Synthetic Augmentation (N={int(total_n)} total samples)", fontsize=13, y=1.02)
        fig.tight_layout()
...
def plot_accuracy_delta_vs_synth(summary_df: pd.DataFrame, output_dir: Path):
    """Shows accuracy improvement (delta) over baseline FedAvg (synth=0).
    Clearly demonstrates diminishing/negative returns for balanced data.
    """
    apply_thesis_style()
    df = _filter_standard_runs(summary_df)
    # Only use no-DP runs for the clean story
    ...
    fig.suptitle("Accuracy Gain from Synthetic Augmentation (No DP)", fontsize=13, y=1.02)
    fig.tight_layout()
...
def plot_accuracy_vs_alpha_by_synth(summary_df: pd.DataFrame, output_dir: Path):
    """Bar/line chart: x=alpha (ordered), y=accuracy, grouped by synthetic_count.
    Faceted by total_n. Shows that augmentation effect varies by distribution.
    All subplots share the same x-axis (ALPHA_ORDER) for consistency.
    """
    apply_thesis_style()
    df = _filter_standard_runs(summary_df)
    df = df[df["target_epsilon_label"] == "none"]
    ...
    # Fixed x-axis positions for all subplots (consistent across all N values)
    ...
    fig.suptitle("Accuracy by Distribution Setting and Synthetic Count (No DP)", fontsize=13, y=1.02)
    fig.tight_layout()
    fig.savefig(output_dir / "accuracy_vs_alpha_by_synth.png", bbox_inches="tight")
    plt.close(fig)


# --- PLOT 3b: Heatmap — rows=synth_count, cols=epsilon, panels per alpha ---

# Epsilon columns: no DP → strongest DP (left to right)
# EPS_COLS and EPS_LABELS are imported from thesis_style


def plot_heatmap_synth_alpha_dp(summary_df: pd.DataFrame, output_dir: Path):
    """One figure per total_n; subplots per alpha.
    Each subplot: rows=synthetic_count, cols=epsilon (none→1), viridis colormap.
    Matches the style: white annotations, subtle grid, per-panel color scale.
    """
    df = _filter_standard_runs(summary_df)
    if df.empty:
        return
    ...
    for total_n, n_df in df.groupby("total_n"):
        ...
        # 7" per panel matches the single-panel figsize from reference style
        ...
        for panel_idx, (ax, alpha) in enumerate(zip(axes, alphas_present)):
            ...
            # Build matrix: rows=synth_count, cols=epsilon
            ...
            # Force all text white regardless of seaborn defaults
        ...
        fig.suptitle(
            f"Mean Accuracy Heatmap  |  N = {int(total_n)}",
...
def plot_dp_impact(summary_df: pd.DataFrame, output_dir: Path):
    """For a fixed synthetic count (e.g. 10), show how DP epsilon affects accuracy
    across different alpha settings. One figure per total_n.
    """
    apply_thesis_style()
    df = _filter_standard_runs(summary_df)
    # Use synth=10 as representative augmented scenario
    ...
    # Order epsilon: No DP → highest ε → lowest ε (higher ε ≈ less noise ≈ closer to no DP)
    ...
    for ax, total_n in zip(axes, total_ns):
        ...
        for alpha in alphas_present:
            ...
            if a_df.empty:
                continue
            # Map epsilon to numeric x for plotting
    ...
    fig.suptitle("DP Impact on Augmented FL (synth=10 per client)", fontsize=13, y=1.02)
    fig.tight_layout()
...
def plot_updates_dp_comparison(summary_df: pd.DataFrame, output_dir: Path):
    """Grouped bar chart comparing standard FedAvg (no DP) vs DP on model updates
    at different update-DP epsilon values. One subplot per total_n.
    """
    apply_thesis_style()

    # Standard runs: synth=0, no DP, updates_dp_enabled=False
    std = summary_df[
    ...
    # DP-updates runs: updates_dp_enabled=True
    dpup = summary_df[summary_df["updates_dp_enabled"] == True].copy()

    if dpup.empty:
        return

    # Get unique update-DP epsilon values, ordered: highest (least noise) → lowest
    ...
    # One bar for standard + one bar per update-DP epsilon
    n_bars = 1 + len(upd_eps_values)
    bar_width = 0.8 / n_bars

    # Colors: green for standard, shades of red for DP-updates (lighter = higher ε)
    ...
    for ax, total_n in zip(axes, total_ns):
        ...
        # One group of bars per update-DP epsilon
    ...
    fig.suptitle(
        "Impact of DP on Model Updates (synth=0, no VAE DP)",
        fontsize=13, y=1.02,
    )
    fig.tight_layout()
...
def cleanup_obsolete_plots(output_dir: Path):
    """Remove plot types that are no longer exported."""
    obsolete_patterns = [
...
def load_study_data(
    *,
    ...
    raw_path = output_dir / "mnist_dpfedaug_seeded_raw.csv"
    ...
    summary_df.to_csv(output_dir / "mnist_dpfedaug_seeded_summary.csv", index=False)
...
def export_study(
    *,
    ...
    print(f"Seeded MNIST plots generated in: {output_dir}")
    return output_dir
